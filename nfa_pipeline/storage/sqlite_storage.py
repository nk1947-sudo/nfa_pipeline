...
class SQLiteStorage:
    ...
    def _create_schema(self) -> None:
        ...
        existing = {row[1] for row in conn.execute("PRAGMA table_info(award_records)").fetchall()}
        for col_name, col_type in new_columns:
            if col_name not in existing:
                conn.execute(f"ALTER TABLE award_records ADD COLUMN {col_name} {col_type}")
                logger.debug("Migrated: added column %s to award_records", col_name)
        conn.commit()
    ...
    def _record_to_row(r: AwardRecord) -> dict:
        ...
        return {
            ...
            "imdb_id":             r.imdb_id,
            "imdb_rating":         str(r.imdb_rating) if r.imdb_rating is not None else None,
            "imdb_votes":          r.imdb_votes,
            "runtime_minutes":     r.runtime_minutes,
            "genres":              r.genres,
            "box_office_worldwide":r.box_office_worldwide,
            "release_date":        r.release_date,
            "is_sequel":           _bool_to_int(r.is_sequel),
            "is_remake":           _bool_to_int(r.is_remake),
