...
# Columns in output order: (field_key, header_label, column_width)
_COLUMNS = [
    ...
    ("imdb_id",              "IMDb ID",           12),
    ("imdb_rating",          "IMDb Rating",       12),
    ("imdb_votes",           "IMDb Votes",        14),
    ("runtime_minutes",      "Runtime (min)",     14),
    ("genres",               "Genres",            24),
    ("box_office_worldwide", "Box Office (WW)",   18),
    ("release_date",         "Release Date",      16),
    # New fields
    ("is_sequel",            "Sequel?",           10),
    ("is_remake",            "Remake?",           10),
    ("crew_size",            "Crew Size",         12),
    # Provenance
...
class ExcelExporter:
    ...
    def __init__(self, storage_cfg, paths_cfg, out_path: Optional[Path] = None):
        ...
        self._out_path = out_path or Path(paths_cfg.output_excel)
    ...
    def export(
        ...
        def _safe_val(v):
            """Truncate strings that would exceed Excel's cell character limit."""
            if isinstance(v, str) and len(v) > _EXCEL_MAX:
                return v[:_EXCEL_MAX - 3] + "..."
            return v
        ...
        for row_idx, rec in enumerate(records, start=2):
            ...
            for col_idx, key in enumerate(col_keys, start=1):
                ...
                if key == "is_valid":
                    val = "✓" if val else "✗"
                elif key in ("is_sequel", "is_remake"):
                    if val is True:
                        val = "Yes"
                    elif val is False:
                        val = "No"
    ...
    def _write_invalid_sheet(self, wb, invalid_records, header_fill, header_font, data_font):
        ...
        for rec in invalid_records:
            ...
            warns = rec.get("validation_warnings", "")
