...
from .storage import SQLiteStorage, ExcelExporter, CSVExporter
...
class NFAPipeline:
    ...
    def run(
        ...
        # ── Stage 5: Store ────────────────────────────────────────────────────
        self._store(all_records, batch_report)
    ...
    def _store(self, records: List[AwardRecord], report) -> None:
        ...
        if self._cfg.storage.excel.enabled:
            try:
                exporter = ExcelExporter(self._cfg.storage.excel, self._cfg.paths)
                exporter.export(record_dicts, validation_report=report)
            except ImportError:
                logger.warning("openpyxl not installed — skipping Excel export")

        # CSV
        if self._cfg.storage.csv.enabled:
            csv_exp = CSVExporter(self._cfg.storage.csv, self._cfg.paths)
            csv_exp.export(record_dicts)
